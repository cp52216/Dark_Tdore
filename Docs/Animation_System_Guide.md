...
### ABP_Character_Base 蓝图变量

| 变量 | 类型 | 来源 |
|------|------|------|
| `GroundSpeed` | float | C++ NativeGroundSpeed |
| `Direction` | float | C++ NativeDirection |
| `ShouldMove` | bool | C++ bNativeShouldMove |
| `IsFalling` | bool | C++ bNativeIsFalling |
| `bHasWeapon` | bool | C++ 自动计算 |
...
### ABP_ItemAnimLayers_Gloves

**位置**: `Content/ThirdPerson/Animations/LinkedLayers/ABP_ItemAnimLayers_Gloves.uasset`

- **继承**: `UDark_TdoreAnimInstance`（直接继承）
- 预留，待补充拳套动画资源
...
└── UDark_TdoreAnimInstance (C++ 基类: 数据桥接)
    ├── ABP_Character_Base
    │   └── ABP_ItemAnimLayers_Sword  ← ❌ 之前这样（Base 不生效）
    │
    └── ABP_ItemAnimLayers_Sword      ← ✅ 现在这样（直接继承）
    └── ABP_ItemAnimLayers_Gloves     ← ✅ 现在这样（直接继承）
```
