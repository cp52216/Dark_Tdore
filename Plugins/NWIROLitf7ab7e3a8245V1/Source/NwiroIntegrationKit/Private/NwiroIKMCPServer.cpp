...
FString FNwiroIKMCPServer::GetToolDefinitionsJson()
{
	...
	J += TEXT("{\"name\":\"edit_blueprint\",\"description\":\"Edit Blueprint. ALL nodes and connections MUST be in ONE call. Node types: CallFunction(function:'PrintString'),Event(event:'ReceiveBeginPlay'),CustomEvent,VariableGet,VariableSet,Branch,Sequence,ForLoop,ForEachLoop,WhileLoop,Gate,DoOnce,FlipFlop,Delay,PrintString,SetTimer,DestroyActor,SetActorLocation,GetActorLocation,SpawnActor(Class pin needs TSubclassOf<Actor>: use type:'class' subtype:'ActorClassName' variable or set_pin_defaults with class name). To access a component on a spawned/external actor use GetProperty NOT CallFunction: type:'GetProperty',property:'StaticMeshComponent',ownerClass:'StaticMeshActor' — Target input pin, output pin named after the property (not ReturnValue), pure node (no exec pins). To get a component from self use GetComponent: type:'GetComponent',component:'CharacterMovement' — pure. Common pins: execute/then (exec flow), ReturnValue, self/Target. VariableGet: output pin is the variable name not ReturnValue. PrintString: execute,InString,then. Event: then. Branch: execute,Condition,True,False. Delay: execute,Duration,Completed.\",\"inputSchema\":{\"type\":\"object\",\"properties\":{\"blueprint\":{\"type\":\"string\"},\"graph\":{\"type\":\"string\"},\"add_nodes\":{\"type\":\"array\",\"items\":{\"type\":\"object\",\"properties\":{\"ref\":{\"type\":\"string\",\"description\":\"Unique ID for this node, used in connect_pins\"},\"type\":{\"type\":\"string\",\"description\":\"CallFunction, Event, CustomEvent, VariableGet, Branch, etc.\"},\"function\":{\"type\":\"string\",\"description\":\"For CallFunction: PrintString, Delay, etc.\"},\"event\":{\"type\":\"string\",\"description\":\"For Event: ReceiveBeginPlay, ReceiveTick\"},\"target\":{\"type\":\"string\",\"description\":\"For CallFunction: component instance name (e.g. 'FirstPersonCamera') or class name to search when resolving the function. Use for component/object functions like GetComponentLocation, GetComponentRotation, GetWorldLocation, or SetSimulatePhysics; avoids brute-force hitting the wrong class.\"}},\"required\":[\"ref\",\"type\"]}},\"connect_pins\":{\"type\":\"array\",\"items\":{\"type\":\"object\",\"properties\":{\"from\":{\"type\":\"string\",\"description\":\"ref.PinName e.g. mynode.then\"},\"to\":{\"type\":\"string\",\"description\":\"ref.PinName e.g. other.execute\"}},\"required\":[\"from\",\"to\"]}},\"set_pin_defaults\":{\"type\":\"array\",\"items\":{\"type\":\"object\",\"properties\":{\"ref\":{\"type\":\"string\"},\"pin\":{\"type\":\"string\"},\"value\":{\"type\":\"string\"}},\"required\":[\"ref\",\"pin\",\"value\"]}},\"add_variables\":{\"type\":\"array\",\"items\":{\"type\":\"object\"}},\"remove_variables\":{\"type\":\"array\",\"items\":{\"description\":\"Variable name (string) or object with a 'name' field\"}},\"add_components\":{\"type\":\"array\",\"items\":{\"type\":\"object\"}},\"compile\":{\"type\":\"boolean\"}},\"required\":[\"blueprint\"]}},");
...
TSharedPtr<FJsonObject> FNwiroIKMCPServer::HandleToolsCall(const TSharedPtr<FJsonObject>& Params)
{
	...
	else
	{
		...
		AsyncTask(ENamedThreads::GameThread, [&ToolResult, &ToolName, &ArgsJson, DoneEvent]()
		{
			ToolResult = DispatchTool(ToolName, ArgsJson);
			DoneEvent->Trigger();
		});
...
FString FNwiroIKMCPServer::DispatchTool(const FString& ToolName, const FString& ArgsJson)
{
	...
	if (ToolName == TEXT("search_assets") && UNwiroIKBridge::Instance)
		return UNwiroIKBridge::Instance->SearchAssets(ArgsJson);
	...
	if (ToolName == TEXT("rename_file"))
	{
		...
		if (FJsonSerializer::Deserialize(R, Args) && Args.IsValid())
		{
			FString OldPath = Args->GetStringField(TEXT("old_path"));
			FString NewPath = Args->GetStringField(TEXT("new_path"));
