...
FString FNwiroIKSequencerTools::CreateSequence(const FString& JsonCommand)
{
	...
	{
		Tx.Cancel();
		return TEXT("{\"success\":false,\"error\":\"Failed to create LevelSequence\"}");
	}
...
FString FNwiroIKSequencerTools::AddSequenceTrack(const FString& JsonCommand)
{
	...
	// Parse binding GUID
	FGuid BindingGuid;
	FGuid::Parse(BindingStr, BindingGuid);
	...
	else if (TrackLower == TEXT("cameraCut") || TrackLower == TEXT("camera_cut") || TrackLower == TEXT("cameracut"))
	{
