...
FString FNwiroIKAssetTools::ReadAsset(const FString& JsonCommand)
{
	...
	if (Path.IsEmpty()) Path = Cmd->GetStringField(TEXT("name"));
	...
	if (!Asset)
	{
		...
		for (const FAssetData& A : Assets)
		{
			if (A.AssetName.ToString().Equals(Path, ESearchCase::IgnoreCase) ||
				A.AssetName.ToString().Contains(Path, ESearchCase::IgnoreCase))
			{
				Asset = A.GetAsset();
				break;
			}
...
FString FNwiroIKAssetTools::GetAssetThumbnail(const FString& JsonCommand)
{
	...
	if (!Thumbnail || Thumbnail->GetImageWidth() == 0)
		return TEXT("{\"success\":false,\"error\":\"Failed to generate thumbnail\"}");
	...
	return FString::Printf(TEXT("{\"success\":true,\"path\":\"%s\",\"width\":%d,\"height\":%d}"),
		*SavePath.Replace(TEXT("\\"), TEXT("/")), Thumbnail->GetImageWidth(), Thumbnail->GetImageHeight());
}
