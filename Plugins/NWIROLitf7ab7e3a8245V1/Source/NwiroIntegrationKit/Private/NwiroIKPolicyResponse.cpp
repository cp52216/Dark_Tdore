...
bool JsonFieldIsType(const TSharedPtr<FJsonObject>& Obj, const FString& Key, EJson Type)
{
	...
	return Found && Found->IsValid() && (*Found)->Type == Type;
}
...
)
{
	...
	for (int32 Index = 0; Index < Arr->Num(); ++Index)
	{
		...
		for (const auto& Pair : ItemObj->Values)
		{
			...
			{
				const FString Field = FString::Printf(TEXT("%s[%d].%s"), *Op, Index, *Pair.Key);
				Errors.Add(MakePolicyDiagnostic(TEXT("UNKNOWN_KEY"), Op, Field, Pair.Key, AllowedList));
			}
		}
