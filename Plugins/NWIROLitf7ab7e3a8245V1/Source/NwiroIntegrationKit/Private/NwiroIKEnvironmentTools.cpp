...
FString FNwiroIKEnvironmentTools::SetPostProcess(const FString& JsonCommand)
{
	...
	if (!PPV) return TEXT("{\"success\":false,\"error\":\"PostProcessVolume not found\"}");
...
FString FNwiroIKEnvironmentTools::SetFog(const FString& JsonCommand)
{
	...
	if (!Fog || !Fog->GetComponent()) return TEXT("{\"success\":false,\"error\":\"ExponentialHeightFog not found\"}");
...
FString FNwiroIKEnvironmentTools::AddSplinePoint(const FString& JsonCommand)
{
	...
	if (!Spline) return TEXT("{\"success\":false,\"error\":\"No spline component\"}");
