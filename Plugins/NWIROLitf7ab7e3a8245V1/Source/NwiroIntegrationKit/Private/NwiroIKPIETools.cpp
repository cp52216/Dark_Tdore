...
static AActor* FindPIEActor(UWorld* World, const FString& Name)
{
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		if (A && (A->GetName() == Name || A->GetActorLabel() == Name ||
			A->GetName().Contains(Name, ESearchCase::IgnoreCase) ||
...
static AAIController* FindAIController(UWorld* World, const FString& ActorName)
{
	...
	// If it's a pawn, get its controller
...
FString FNwiroIKPIETools::PIETeleportActor(const FString& JsonCommand)
{
	...
	return FString::Printf(TEXT("{\"success\":%s,\"actor\":\"%s\",\"position\":\"(%.0f, %.0f, %.0f)\"}"),
		bSuccess ? TEXT("true") : TEXT("false"), *Actor->GetName(), X, Y, Z);
}
...
FString FNwiroIKPIETools::PIESpawnActor(const FString& JsonCommand)
{
	...
	return FString::Printf(TEXT("{\"success\":true,\"actor\":\"%s\",\"class\":\"%s\"}"),
