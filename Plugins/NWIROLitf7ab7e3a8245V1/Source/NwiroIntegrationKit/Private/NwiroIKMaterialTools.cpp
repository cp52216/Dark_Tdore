	...
	bool& bOutKeyPresentButEmpty)
{
	...
	for (const auto& Pair : Obj->Values)
	{
		...
		if (!RecognizedAliasNorms.Contains(NormalizeKey(Pair.Key))) continue;
		return MarkResult(Pair.Key, Pair.Value->AsString());
	}
...
FString FNwiroIKMaterialTools::UpdateMaterialInstance(const FString& JsonCommand)
{
	...
	FString MIPath = Cmd->GetStringField(TEXT("path"));
	if (MIPath.IsEmpty())
	{
		return TEXT("{\"success\": false, \"error\": \"Missing 'path' field\"}");
	}
...
FString FNwiroIKMaterialTools::ApplyMaterial(const FString& JsonCommand)
{
	...
	// Accept both camelCase (schema) and snake_case (legacy) for each param
	FString MatPath, ActorTarget;
	if (!Cmd->TryGetStringField(TEXT("materialPath"), MatPath))
		Cmd->TryGetStringField(TEXT("material_path"), MatPath);
	if (!Cmd->TryGetStringField(TEXT("actorPath"), ActorTarget))
		if (!Cmd->TryGetStringField(TEXT("actor_target"), ActorTarget))
			Cmd->TryGetStringField(TEXT("actorName"), ActorTarget);
	...
	{
		return TEXT("{\"success\": false, \"error\": \"Missing required fields: materialPath, actorPath\"}");
	}
	...
	for (TActorIterator<AActor> It(World); It; ++It)
	{
		...
		// Check full path first (most specific)
		...
		// Fall back to actor label (display name in the outliner)
		if (Actor->GetActorLabel() == ActorTarget)
		{
			TargetActor = Actor;
			// Don't break - a path match later in iteration should win
		}

		// Fall back to internal actor name
		if (!TargetActor && Actor->GetName() == ActorTarget)
		{
			TargetActor = Actor;
		}
	}
...
void FNwiroIKMaterialTools::SetExpressionProperties(UMaterialExpression* Expr, const TSharedPtr<FJsonObject>& Props, TArray<FString>* OutWarnings, TArray<TSharedPtr<FJsonValue>>* OutErrors, TArray<FString>* OutUnknownPropertyKeys, TMap<FString, FString>* OutAssetMissPaths)
{
	...
	{
		const FString NKeyEarly = NormalizeKey(Pair.Key);
		...
		const FString NKey = NormalizeKey(Pair.Key);
		...
		if (NKey == TEXT("texture"))
		{
			...
			else
			{
				...
				{
					OutAssetMissPaths->Add(Pair.Key, TexPath);
				}
				else if (OutErrors)
				{
		...
		if (!Prop)
		{
			...
			if (OutUnknownPropertyKeys) OutUnknownPropertyKeys->Add(Pair.Key);
			continue;
		}
		...
		else if (FObjectProperty* ObjProp = CastField<FObjectProperty>(Prop))
		{
			...
			else
			{
				...
				if (OutAssetMissPaths)
				{
					OutAssetMissPaths->Add(Pair.Key, AssetPath);
				}
				else if (OutErrors)
				{
					OutErrors->Add(MakeDiagnostic(
...
FNwiroIKMatResult FNwiroIKMaterialTools::DoConnectExpressions(UMaterial* Mat, const TArray<TSharedPtr<FJsonValue>>& Items)
{
	...
	for (int32 i = 0; i < Items.Num(); i++)
	{
		...
		{
			// Fallback: search by UE5 expression name in the material
			for (const TObjectPtr<UMaterialExpression>& E : Mat->GetExpressionCollection().Expressions)
			{
				if (E && E->GetName() == FromRef)
				{
					ExpressionRefs.Add(FromRef, E); // cache for future lookups
...
FString FNwiroIKMaterialTools::FindMaterialFunctions(const FString& JsonCommand)
{
	...
	{
		if (!AllowedTopLevel.Contains(Pair.Key))
		{
			ValidationErrors.Add(MakePolicyDiagnostic(TEXT("UNKNOWN_KEY"), TEXT("schema"), Pair.Key, Pair.Key, AllowedList));
		}
	}
	...
	TArray<TSharedPtr<FJsonValue>>& Errors)
{
	...
	for (int32 Index = 0; Index < Arr->Num(); ++Index)
	{
		...
		{
			const TSharedPtr<FJsonValue>* Found = ItemObj->Values.Find(KT.Key);
...
FString FNwiroIKMaterialTools::EditMaterialFunction(const FString& JsonCommand)
{
	...
	for (const auto& Pair : Cmd->Values)
	{
		if (!AllowedTopLevel.Contains(Pair.Key))
		{
			ValidationErrors.Add(MakePolicyDiagnostic(TEXT("UNKNOWN_KEY"), TEXT("schema"), Pair.Key, Pair.Key, AllowedTopLevelList));
		}
	}
	...
	auto NormalizeArrayField = [&](const FString& Key)
	{
		...
		const TSharedPtr<FJsonValue>* Val = Cmd->Values.Find(Key);
	...
	if (Cmd->TryGetObjectField(TEXT("setMetadata"), MetaObj) && MetaObj)
	{
		...
		for (const auto& Pair : (*MetaObj)->Values)
		{
			...
			{
				ValidationErrors.Add(MakePolicyDiagnostic(TEXT("UNKNOWN_KEY"), TEXT("setMetadata"), FString::Printf(TEXT("setMetadata.%s"), *Pair.Key), Pair.Key, MetadataAllowed));
			}
		}
...
FString FNwiroIKMaterialTools::DeleteMaterialFunction(const FString& JsonCommand)
{
	...
	auto NormalizeStringBoolField = [&](const FString& Key)
	{
		if (!Cmd->HasField(Key)) return;
		const TSharedPtr<FJsonValue>* Val = Cmd->Values.Find(Key);
		if (!Val || !Val->IsValid() || (*Val)->Type != EJson::String) return;
